...
class Zones extends Resource {
   ...
   */
  async browse(zoneId: string | null = null, per_page: number = 20, page: number = 1) {
    const params: any = {};
    if (per_page > 50) {
   ...
   * @returns {Promise<Object>} The zone browse response object.
   */
  async del(zoneId: string | null = null) {
    const params: any = {};
    params.zoneId = zoneId;
    return await this.apiClient.request(
      {
   ...
   * @param {string} [zoneId] - zoneId
   * */
  async activationCheck(zoneId: string) {
    const params: any = {};
    params.zoneId = zoneId;
    return await this.apiClient.request(
      {
   ...
   */
  purgeCache(zoneId: string, data: any) {
    const params: any = {};
    params.zoneId = zoneId;
